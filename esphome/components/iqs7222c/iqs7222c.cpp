...
void IQS7222CComponent::iqs_7222c_hal_init(void) {
  // gpio_config_t io_conf;

  // system_i2c_master_init(IQS_7222C_I2C_NUM, IQS_7222C_I2C_SDA_GPIO_NUM, IQS_7222C_I2C_SCL_GPIO_NUM, 100000);
  // /* disable interrupt */
  // io_conf.intr_type = GPIO_INTR_DISABLE;
  // /* set as output mode */
  // io_conf.mode = GPIO_MODE_OUTPUT;
  // /* bit mask of the pins that you want to set */
  // io_conf.pin_bit_mask = IQS_7222C_MCLR_PIN_SEL;
  ...
  // /* bit mask of the pins that you want to set */
  // io_conf.pin_bit_mask = IQS_7222C_RDY_PIN_SEL;
  // /* configure GPIO with the given settings */
  // gpio_config(&io_conf);
}

uint8_t IQS7222CComponent::iqs_7222c_rdy_read(void) {
  return this->interrupt_pin_->digital_read();
  // return gpio_get_level(IQS_7222C_RDY_PIN_NUM);
}
/**
...
void IQS7222CComponent::iqs_7222c_mclr_set(void) {
  this->mclr_pin_->digital_write(0);
  // IQS7222CComponent::gpio_set_level(IQS_7222C_MCLR_PIN_NUM, 0);
}

void IQS7222CComponent::iqs_7222c_mclr_clear(void) {
  this->mclr_pin_->digital_write(1);
  //  IQS7222CComponent::gpio_set_level(IQS_7222C_MCLR_PIN_NUM, 1);
}

void IQS7222CComponent::iqs_7222c_delay(uint32_t ms) {
  delay(ms);
...
void IQS7222CComponent::iqs_7222c_i2c_write_cont(uint8_t *data, uint16_t data_len) {
  ...
  //  this->write_register(0, data, data_len, false);
...
[15:37:35][D][iqs7222c:657]: First run execution. touch hard reset
[15:37:35][D][iqs7222c:300]: iqs_7222c_init
[15:37:35][D][iqs7222c:369]: 722c ch0 setup :0x0203
[15:37:35][D][iqs7222c:378]: 722c ch1 setup :0x0203
[15:37:35][D][iqs7222c:387]: 722c ch2 setup :0x0203
[15:37:35][D][iqs7222c:396]: 722c ch3 setup :0x0203
[15:37:35][D][iqs7222c:405]: 722c ch4 setup :0x0203
[15:37:35][D][iqs7222c:414]: 722c ch5 setup :0x0203
[15:37:35][D][iqs7222c:423]: 722c ch6 setup :0x0203
[15:37:35][D][iqs7222c:432]: 722c ch7 setup :0x0203
[15:37:35][D][iqs7222c:441]: 722c ch8 setup :0x0203
[15:37:35][D][iqs7222c:450]: 722c ch9 setup :0x0203
[15:37:36][W][component:237]: Component iqs7222c took a long time for an operation (1046 ms).
[15:37:36][W][component:238]: Components should block for at most 30 ms.
[15:37:36][D][iqs7222c:722]: Touch event: 0
[15:37:36][D][iqs7222c:727]: Status event: 9
[15:37:36][D][iqs7222c:738]: run() New data available
[15:37:36][D][iqs7222c:727]: Status event: 8
[15:37:41][D][iqs7222c:727]: Status event: 24
[15:37:47][D][iqs7222c:727]: Status event: 40
  ...
  switch (iqs7222c_state.init_state) {
    ...
    case IQS7222C_INIT_VERIFY_PRODUCT:
      if (this->getRDYStatus()) {
        ...
        prod_num = getProductNum(STOP, ver_maj, ver_min);
        // ver_maj = getmajorVersion(RESTART);
        // ver_min = getminorVersion(STOP);
        ...
        } else if (prod_num == 0xeeee || ver_maj == 0xee || ver_min == 0xee) {
          // error from the device. RDY window closed?????
          bool rdy = this->interrupt_pin_->digital_read();
          ESP_LOGD(TAG, "Something is 0xee. RDY pin is %s", rdy ? "HIGH" : "LOW");
 ...
 *         should be displayed.
 */
uint16_t IQS7222CComponent::getProductNum(bool stopOrRestart, uint8_t &major, uint8_t &minor) {
  uint8_t transferBytes[6];    // A temporary array to hold the byte to be transferred.
  uint8_t prodNumLow = 0;      // Temporary storage for the counts low byte.
  uint8_t prodNumHigh = 0;     // Temporary storage for the counts high byte.
  uint16_t prodNumReturn = 0;  // The 16-bit return value.
  ...
  major = transferBytes[2];
  minor = transferBytes[4];
...
void IQS7222CComponent::readRandomBytes(uint8_t memoryAddress, uint8_t numBytes, uint8_t bytesArray[],
                                        bool stopOrRestart) {
  ESP_LOGD(TAG, "readRandomBytes REG=0x%02x, LEN=%d", memoryAddress, numBytes);

  const uint8_t NUM_RETRIES = 5;
  uint8_t retry = 0;
  i2c::ErrorCode ret = i2c::ErrorCode::ERROR_CRC;

  while (retry++ < NUM_RETRIES && ret != i2c::ErrorCode::NO_ERROR) {
    ret = this->read_register(memoryAddress, bytesArray, numBytes, stopOrRestart);
    if (ret != i2c::ErrorCode::NO_ERROR) {
      ESP_LOGE(TAG, "Error reading bytes from memory address %02X, error %d", memoryAddress, (int) ret);
    } else if (numBytes > 0) {
      if (bytesArray[0] == 0xee) {
        ret = i2c::ErrorCode::ERROR_CRC;
      } else
        switch (numBytes) {
          case 1:
            ESP_LOGD(TAG, "readRandomBytes DATA=0x%02x", bytesArray[0]);
            break;
          case 2:
            ESP_LOGD(TAG, "readRandomBytes DATA=0x%02x.%02x", bytesArray[0], bytesArray[1]);
            break;
          case 3:
            ESP_LOGD(TAG, "readRandomBytes DATA=0x%02x.%02x.%02x", bytesArray[0], bytesArray[1], bytesArray[2]);
            break;
          case 4:
            ESP_LOGD(TAG, "readRandomBytes DATA=0x%02x.%02x.%02x.%02x", bytesArray[0], bytesArray[1], bytesArray[2],
                     bytesArray[3]);
            break;
          default:
            ESP_LOGD(TAG, "readRandomBytes DATA=0x%02x.%02x.%02x.%02x.%02x..", bytesArray[0], bytesArray[1],
                     bytesArray[2], bytesArray[3], bytesArray[4]);
            break;
        }
    }
    if (ret != i2c::ErrorCode::NO_ERROR) {
      ...
      if (this->interrupt_pin_->digital_read()) {
        ...
        // ESP_LOGD(TAG, "IQS7222CComponent::readRandomBytes: RDY pin is HIGH, force I2C communication"
        // this->force_I2C_communication();
      }
...
void IQS7222CComponent::writeRandomBytes16(uint16_t memoryAddress, uint8_t numBytes, uint8_t bytesArray[],
                                           bool stopOrRestart) {
  auto ret = this->write_register16(memoryAddress, bytesArray, numBytes, stopOrRestart);
  if (ret != i2c::ErrorCode::NO_ERROR) {
    ESP_LOGE(TAG, "Error writing bytes to memory address %04X, error %d", memoryAddress, (int) ret);
  }
  // // Select the device with the address of "_deviceAddress" and start communication.
  // Wire.beginTransmission(_deviceAddress);
